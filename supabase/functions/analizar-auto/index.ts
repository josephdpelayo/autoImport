...
Con lo que te compartan (datos ya conocidos del auto, texto/link pegado, PDF o foto del listing/Condition Report), da tu análisis en este formato:
...
Deno.serve(async (req) => {
  ...
  try {
    const { vin, texto, pdfBase64, imagenBase64 } = await req.json();
    ...
    content.push({ type: "text", text: "Da tu análisis y veredicto siguiendo el formato indicado." });
